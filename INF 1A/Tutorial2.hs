...
titlecapital :: String -> String
titlecapital (head:tail) | length (head:tail) <4 = allsmall (head:tail)
                         | length (head:tail) >=4 = capitalise (head:tail)

title :: [String] -> [String]
title (head:tail) =  capitalise head :[titlecapital xs | xs <- tail] 

eds :: [String]
eds = ["tHe", "sOunD", "ANd", "thE", "FuRY"]
...
crosswordFind letter pos len words = [xs | xs <- words, length xs == len,  letterpos letter pos xs, pos >= 0, len > pos]

letterpos :: Char -> Int -> String -> Bool
letterpos letter pos word= word !! pos == letter

wordes = ["funky", "fabulous", "kite", "icky", "ukelele"]
-- 6. search
