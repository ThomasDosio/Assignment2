...
columnsComplete = And [ Or [ P (i,j,n) | i <- [1..9]] | j <- [1..9], n <- [1..9]]

squaresComplete :: Form (Int,Int,Int)
squaresComplete = And [Or [ P (i,j,n) | i <- [(3*a+1)..(3*a+3)], j <- [(3*b+1)..(3*b+3)]] | a <- [0,1,2], b <- [0,1,2], n <- [1..9]]
...
--squaresNoRepetition = And [Or ([] ++ [ P (i,j,n) | i <- [(3*a+1)..(3*a+3)], j <- [(3*b+1)..(3*b+3)], n <- [1..9]]) | a <- [0,1,2], b <- [0,1,2]]
--squaresNoRepetition = And [Or [N(i, j, n), N(x, y, n)] |  a <- [0..2], b <- [0..2],n <- [1..9], i <- [3*a+1..3*a+3], j <- [3*b+1..3*b+3], x <- [i..i+2], y<- [j,j+1,j+2]]
squaresNoRepetition = And [Or [(N (i,j,n)), k ]| n <- [1..9], i <- [1..9], j <- [1..9], k <- (allOther (N (i,j,n))) ] 


allOther (N (i,j,n)) = removegiven (N (i,j,n)) [N(x,y,n) | x<- (group2 i), y <- (group2 j)]

group2 k | k<= 3 = [1,2,3]
         | k> 3 && k<= 6 = [4,5,6]
         | k > 6 && k<=9 = [7,8,9]
         | otherwise = error "I can't be bothered writing an error message but you've made a mistake"

removegiven :: Literal (Int, Int, Int) -> [Literal (Int, Int, Int)] -> [Literal (Int, Int, Int)]
removegiven _ [] = []
removegiven (N(i,j,n)) (N(x,y,v): remaining) | i==x && j==y && n==v = remaining
                                             | otherwise = removegiven (N(i,j,n)) remaining
...
The constraint of having the rows be complete is redundant because if there are no empty or double filled squares and no repetitions then every entry of the rows will contain a different entry and so will be complete. 
The same is also true of having the columns be complete or the squares be complete, so a minimal amd complete description can be given simply by saying that every square has exactly one number and no rows, columns or squares have repetition.
The more constraints you have the more efficient the program becomes because it's search space is more limited. 
For example printing the first solution without using rowsComplete takes 58.55 seconds  whereas if one includes the constraint the same answer is returned in 15.51 seconds and the number of bytes used is roughly a quarter.
Removing both columns-complete and rows-complete takes 462.38 seconds which is an even larger ammount of time. 
Removing the constraint that the squares are complete leads to a perhaps surprising result: the time taken doesn't change by much however the result is still correct. It is however still marginally longer so I would include it.
Removing all three completeness constraints leads to a longer time frame but still correct results. It took it  461.62 seconds to get the correct answer which is again very similar to the time frame for when I removed just rows and columns therefore leading to the conclusion that sqaures complete doesnt actually add anything of value to the program's efficiency.
-}
